...
export class Project {
  ...
  public isClassDeclarationForRouterModule(
  ...
  public getRouterModule(): NgModule | undefined {
    return this.builtInNgModules.find(
      ngModule => ngModule.getName() == 'RouterModule'
    );
  }
  ...
  constructor(
    public readonly tsMorphProject: tsMorph.Project,
  ...
  public getNgModuleByClassNameOrThrow(className: string): NgModule {
    return throwIfUndefined(
      this.getNgModuleByClassName(className),
      `Expected to find an NgModule with class name "${className}".`
    );
  }

  public getNgModuleWhereDeclared(
    classDeclaration: ClassDeclaration
  ): NgModule | undefined {
    return this.getNgModules().find(ngModule => {
      return ngModule
        ...
        .some(
          declaration => declaration.getClassDeclaration() == classDeclaration
        );
    });
  }

  public getNgModuleWhereDeclaredOrThrow(
    classDeclaration: ClassDeclaration
  ): NgModule {
    return throwIfUndefined(
      this.getNgModuleWhereDeclared(classDeclaration),
      `Expected to find "${classDeclaration.getName()}" among declarables.`
    );
  }
  ...
  public getComponentsByClassName(className: string): Component[] {
    ...
    return flatMap(ngModules, ngModule =>
      ngModule
        .getComponents()
        .filter(component => component.getName() == className)
    );
  }
  ...
  public getComponentByClassNameIfSingleOrThrow(className: string): Component {
    return throwIfLengthNotOne(
      this.getComponentsByClassName(className),
      actual =>
        `Expected exactly 1 component with class name "${className}" in the project but got ${actual}.`
    );
  }

  public getNgModuleByClassDeclaration(
    classDeclaration: ClassDeclaration
  ): NgModule | undefined {
    return this.registeredNgModules.find(
      mod => mod.getClassDeclaration() == classDeclaration
    );
  }

  public isNgModuleRegistered(
    ngModuleClassDeclaration: ClassDeclaration
  ): boolean {
    const existingNgModule = this.getNgModuleByClassDeclaration(
      ngModuleClassDeclaration
    );
  ...
  public registerNgModuleButThrowIfRegistered(ngModule: NgModule): NgModule {
    ...
    if (existingNgModule != null) {
      ...
      throw new Error(
        `NgModule for class declaration "${name}" in ${filePath} has already been registered to the project.`
      );
    }
  ...
  public getDeclarableByClassDeclaration(
    classDeclaration: ClassDeclaration
  ): Declarable | undefined {
    return this.allDeclarables.find(
      dec => dec.getClassDeclaration() == classDeclaration
    );
  }

  public isDeclarableRegistered(
    declarableClassDeclaration: ClassDeclaration
  ): boolean {
    const existingDeclarable = this.getDeclarableByClassDeclaration(
      declarableClassDeclaration
    );
    return existingDeclarable != null;
  }

  public registerDeclarable(
    declarableClassDeclaration: ClassDeclaration,
    ngModule: NgModule
  ): Declarable {
    for (const existingDeclarable of this.allDeclarables) {
      if (
        declarableClassDeclaration == existingDeclarable.getClassDeclaration()
      ) {
        const declarableName = existingDeclarable.getName();
        const ngModuleName = existingDeclarable.getNgModule().getName();
        const filePath = existingDeclarable
          .getClassDeclaration()
          .getSourceFile()
          .getFilePath();
        throw new Error(
          `Declarable "${declarableName}" in "${filePath}" has already been declared in ${ngModuleName}. Cannot declare it in ${ngModule.getName()}.`
        );
      }
    }
    const declarable = createDeclarable(
      this,
      ngModule,
      declarableClassDeclaration
    );
  ...
  public getRootRoutesOrThrow(): Routes {
    return throwIfUndefined(
      this.getRootRoutes(),
      `Expected to find root routes in the project.`
    );
  }

  private setUpBuiltInNgModules() {
    const specifiers: string[] = [`router/router.d.ts#RouterModule`];
    specifiers.forEach(specifier => {
      const [moduleName, className] = specifier.split('#');
      const [root] = this.tsMorphProject.getRootDirectories();
      const fullPath = path.join(
        root.getPath(),
        `./node_modules/@angular/${moduleName}`
      );
      ...
      const classDeclarations = file.getDescendantsOfKind(
      ...
      if (classDeclaration == null) {
        console.warn(
          `Class declaration "${className}" not found in file "${fullPath}". Skipping...`
        );
        return;
      }
  ...
  private setUp() {
    ...
    if (bootstrapModuleIdentifier == null) {
      throw new Error(
        `Expected to find a platformBrowserDynamic().bootstrapModule(xxx) call.`
      );
    }

    if (!TypeGuards.isIdentifier(bootstrapModuleIdentifier)) {
      throw new Error(
        `Expected bootstrapModule to have an identifier as the first argument.`
      );
    }
    ...
    if (classDeclaration == null) {
      throw new Error(
        `Expected bootstrapModule's first argument to reference a class.`
      );
    }
