...
export function throwIf<C>(
  predicate: (x: any) => x is C
): <T>(x: T, msg?: string) => Exclude<T, C>;
export function throwIf<C>(
  guard: (x: any) => boolean
): <T>(x: any, msg?: string) => Exclude<T, undefined>;
...
export function throwIfLengthNotOne<T>(
  array: T[],
  msg?: (actual: number) => string
): T {
  if (array.length == 1) {
    return array[0];
  } else {
    throw new Error(
      msg == null
        ? `Expect exactly 1 elements but got ${array.length}.`
        : msg(array.length)
    );
  }
}
 ...
 * @param kind
 */
export function getPropertyValueOfKind<TKind extends tsm.SyntaxKind>(
  object: tsm.ObjectLiteralExpression,
  propName: string,
  kind: TKind
): tsm.KindToNodeMappings[TKind] | undefined {
...
export function getPropertyValueOfKindOrThrow<TKind extends tsm.SyntaxKind>(
  object: tsm.ObjectLiteralExpression,
  propName: string,
  kind: TKind
): tsm.KindToNodeMappings[TKind] {
  return throwIfUndefined(
    getPropertyValueOfKind(object, propName, kind),
    `Expected to find "${propName}" in ${object.getText()}.`
  );
}
...
function concatErrors(
  ...
  return [mainErrorMessage, customErrorMessage]
...
export function getFirstElementOrThrow<T>(array: T[], err?: string): T {
  const msg = concatErrors(
    `Cannot get the first element of an empty array.`,
    err
  );
...
export function getLastElementOrThrow<T>(array: T[], err?: string): T {
  const msg = concatErrors(
    `Cannot get the last element of an empty array.`,
    err
  );
