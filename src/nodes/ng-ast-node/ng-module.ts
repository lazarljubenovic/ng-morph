...
function getArrayElementsFromObjectLiteralPropertyInitializer(
  object: tsm.ObjectLiteralExpression,
  propertyName: string
): tsm.Node[] {
  const objectLiteralElementLike = object.getProperty(propertyName);
  if (objectLiteralElementLike == null) return [];
  if (!tsm.TypeGuards.isPropertyAssignment(objectLiteralElementLike))
    throw new Error(`Expected @NgModule.imports to be a property assignment.`);
  const initializer = objectLiteralElementLike.getInitializer();
  if (
    initializer == null ||
    !tsm.TypeGuards.isArrayLiteralExpression(initializer)
  )
    throw new Error(
      `Expected @NgModule.imports to be initialized with an array literal expresion.`
    );
  return resolveArrayDestructing(initializer);
}

function getClassDeclarationFromLoadChildrenArrowFunction(
  project: Project,
  arrowFunction: tsm.ArrowFunction
): tsm.ClassDeclaration {
  const innerArrowFunction = arrowFunction.getFirstDescendantByKindOrThrow(
    SyntaxKind.ArrowFunction
  );
  const propAccess = innerArrowFunction.getFirstDescendantByKindOrThrow(
    SyntaxKind.PropertyAccessExpression
  );
  const classDeclarationReference = propAccess.getLastChildIfKindOrThrow(
  ...
  return throwIfUndefined(
    definition,
    `Expected "loadChildren" to point to a class declaration: "${arrowFunction.getText()}".`
  );
}

function readRoutingInfoFromExpression(
  project: Project,
  callExpression: tsm.Expression
): { routes: Route[]; isForRoot: boolean; isForChild: boolean } {
  if (!tsm.TypeGuards.isCallExpression(callExpression)) {
    const text = callExpression.getText();
    const kind = callExpression.getKindName();
    throw new Error(
      `Routing info must be read from a CallExpression, such as "RoutingModule.forRoot(routes)". Instead got "${text}" (which is a ${kind}).`
    );
  }

  const left = callExpression.getExpression();
  if (!tsm.TypeGuards.isPropertyAccessExpression(left))
    throw new Error(`Expected a PropertyAccessExpression`);
  const methodName = left.getNameNode();
  if (!tsm.TypeGuards.isIdentifier(methodName))
  ...
  const arg = throwIfLengthNotOne(
    args,
    actual =>
      `Expected only a single argument in ${callExpression.getText()}, instead got ${actual}.`
  );
  const arrayLiteralExpression = resolveTo(
    arg,
    SyntaxKind.ArrayLiteralExpression
  );

  return {
    routes: readRoutingInfoFromArrayLiteralExpression(
      project,
...
function readRoutingInfoFromArrayLiteralExpression(
  project: Project,
  arrayLiteral: tsm.ArrayLiteralExpression
): Route[] {
  return arrayLiteral.getElements().map(element => {
    if (!tsm.TypeGuards.isObjectLiteralExpression(element)) {
      throw new Error(
        `Expected all elements in route definition to be object literals. Instead got ${element.getKindName()}.`
      );
    }

    const locationSpan = LocationSpan.FromTsm(element);

    const pathProperty = getPropertyValueOfKindOrThrow(
      element,
      'path',
    ...
    const loadChildrenArrowFunction = getPropertyValueOfKind(
      element,
      'loadChildren',
      SyntaxKind.ArrowFunction
    );
    const redirectToStringLiteral = getPropertyValueOfKind(
      element,
      'redirectTo',
      SyntaxKind.StringLiteral
    );
    ...
    if (loadChildrenArrowFunction != null) {
      const ngModuleClassDeclaration = getClassDeclarationFromLoadChildrenArrowFunction(
        project,
        loadChildrenArrowFunction
      );
      const ngModule = project.registerNgModuleOrIgnore(
        new NgModule(project, ngModuleClassDeclaration)
      );
      ...
      const pathMatch: string = getPropertyValueOfKindOrThrow(
        element,
        'pathMatch',
        SyntaxKind.StringLiteral
      ).getLiteralValue();
      if (!tg.isEnum('prefix' as const, 'full' as const)(pathMatch)) {
        throw new Error(
          `Expected "prefix" or "full" for "pathMatch", but got "${pathMatch}".`
        );
      }
      ...
      const childrenArrayLiteral = getPropertyValueOfKind(
        element,
        'children',
        SyntaxKind.ArrayLiteralExpression
      );
      const componentClassDeclaration = getPropertyValueOfKind(
        element,
        'component',
        SyntaxKind.ClassDeclaration
      );
      const ngModule =
        componentClassDeclaration == null
          ? undefined
          : project.getNgModuleWhereDeclared(componentClassDeclaration);
      const component =
        componentClassDeclaration == null
          ? undefined
          : new Component(project, ngModule!, componentClassDeclaration);
      const children =
        childrenArrayLiteral == null
          ? []
          : readRoutingInfoFromArrayLiteralExpression(
              project,
              childrenArrayLiteral!
            );
...
function resolveAsDeveloperDefinedClass(
  project: Project,
  ngModuleAstNode: NgModule,
  ...
  if (decorator == null) {
    ...
    throw new Error(
      `Expected class declaration for ${className} in ${filePath} to have decorator @NgModule.`
    );
  }
  const decoratorArguments = decorator.getArguments();
  if (decoratorArguments.length != 1)
    throw new Error(`Expected @NgModule to to have a single argument.`);
  const [object] = decoratorArguments;
  if (!tsm.TypeGuards.isObjectLiteralExpression(object))
    throw new Error(
      `Expected the first argument of @NgModule to be an object literal.`
    );

  const declarationElements = getArrayElementsFromObjectLiteralPropertyInitializer(
    object,
    'declarations'
  );
  for (const element of declarationElements) {
    if (!tsm.TypeGuards.isIdentifier(element)) {
      throw new Error(
        `Expected all elements in @NgModule.declarations array to be identifiers, found: ${element.getText()}.`
      );
    }
    const elementDefinitions = element.getDefinitionNodes();
    const classDeclaration = elementDefinitions.find(
      tsm.TypeGuards.isClassDeclaration
    );
    if (classDeclaration == null)
      throw new Error(
        `All elements of @NgModule.declarations are expected to reference a class.`
      );
    const newDeclarable = project.registerDeclarable(
      classDeclaration,
      ngModuleAstNode
    );
  ...
  const importElements = getArrayElementsFromObjectLiteralPropertyInitializer(
    object,
    'imports'
  );
  for (const element of importElements) {
    ...
    if (tsm.TypeGuards.isIdentifier(element)) {
      ...
      if (!tsm.TypeGuards.isPropertyAccessExpression(propertyAccessExpression))
        throw new Error(
          `Expected expression in ${element.getText()} to be a property access expression.`
        );
      const identifier = propertyAccessExpression.getExpression();
      if (!tsm.TypeGuards.isIdentifier(identifier))
        throw new Error(
          `Expected an identifier on the left of ${element.getText()}.`
        );
      elementDefinitions = identifier.getDefinitionNodes();
    } else {
      throw new Error(
        `Expected all elements in @NgModule.imports to be identifiers or function calls, found: ${element.getText()}.`
      );
    }
    const classDeclaration = elementDefinitions.find(
      tsm.TypeGuards.isClassDeclaration
    ...
    if (classDeclaration == null) {
      const definitionKindNames = elementDefinitions.map(def =>
        def.getKindName()
      );
      throw new Error(
        `@NgModule.imports found to contain ${element.getText()}, which has ${
          definitionKindNames.length
        } definitions: ${definitionKindNames.join(
          ', '
    ...
    if (
      isCallExpression &&
      project.isClassDeclarationForRouterModule(classDeclaration)
    ) {
    ...
    const existingNgModule = project.getNgModuleByClassDeclaration(
      classDeclaration
    );
  ...
  const exportElements = getArrayElementsFromObjectLiteralPropertyInitializer(
    object,
    'exports'
  );
  for (const element of exportElements) {
    if (!tsm.TypeGuards.isIdentifier(element)) {
      throw new Error(
        `Expected all elements in @NgModule.exports array to be identifiers, found: ${element.getText()}.`
      );
    }
    const elementDefinitions = element.getDefinitionNodes();
    const classDeclaration = elementDefinitions.find(
    ...
    if (classDeclaration == null) {
      const definitionNames = elementDefinitions
        .map(def => `"${def.getText()}" (a ${def.getKindName()})`)
        .join(', ');
      throw new Error(
        `All elements of @NgModule.exports are expected to reference a class. Instead got ${definitionNames}.`
      );
    }
    const alreadyRegisteredNgModule = project.getNgModuleByClassDeclaration(
      classDeclaration
    );
    const alreadyDeclaredDeclarable = project.getDeclarableByClassDeclaration(
      classDeclaration
    );
    if (alreadyRegisteredNgModule != null) {
      ...
      throw new Error(
        `Expected ${classDeclaration.getName()} to have already been registered as either a declarable or a module.`
      );
    }
  }
...
export class NgModule extends NgAstNode {
  ...
  public constructor(
    ...
    if (filePath.includes('/node_modules/')) {
      console.warn(
        `Ignoring ${classDeclaration.getName()} in ${filePath} for now; this is external...`
      );
      this.directImports = [];
      this.exports = [];
      this.declarable = [];
    } else {
      const results = resolveAsDeveloperDefinedClass(
        project,
        this,
        classDeclaration
      );
      ...
      this.routes = new Routes(
        project,
        this.getLocationSpan(),
        results.isForRoot,
        results.isForChild,
        results.routeDefinitions
      );
    }
  }
  ...
  public getComponentOrThrow(
    selector: (component: Component) => boolean
  ): Component {
    return throwIfUndefined(
      this.getComponent(selector),
      `Expected to find a component in NgModule "${this.getName()}".`
    );
  }
  ...
  public getComponentByClassNameOrThrow(className: string): Component {
    return throwIfUndefined(
      this.getComponentByClassName(className),
      `Expected to find a component with class name "${className}" in NgModule "${this.getName()}".`
    );
  }

  public getComponentBySelectorDefinition(
    selectorDefinition: string
  ): Component | undefined {
    return this.getComponent(
      component => component.getSelectorDefinition() == selectorDefinition
    );
  }

  public getComponentBySelectorDefinitionOrThrow(
    selectorDefinition: string
  ): Component {
    return throwIfUndefined(
      this.getComponentBySelectorDefinition(selectorDefinition),
      `Expected to find a component with selector definition "${selectorDefinition}" in NgModule "${this.getName()}".`
    );
  }
  ...
  public getRootRoutesOrThrow(): Routes {
    return throwIfUndefined(
      this.getRootRoutes(),
      `Expected to find root routes visible from ${this.getName()}.`
    );
  }
  ...
  public getChildRoutesOrThrow(): Routes {
    return throwIfUndefined(
      this.getChildRoutes(),
      `Expected to find child routes visible from ${this.getName()}.`
    );
  }
}
