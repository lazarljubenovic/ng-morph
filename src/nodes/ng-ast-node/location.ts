...
export class LocationFileManager extends SimpleCache<
  tsm.SourceFile | string,
  LocationFile
> {
  protected create(
    sourceFileOrAbsolutePath: tsm.SourceFile | string
  ): LocationFile {
    if (typeof sourceFileOrAbsolutePath == 'string') {
      if (!path.isAbsolute(sourceFileOrAbsolutePath)) {
        throw new Error(
          `Path must be absolute. Got: "${sourceFileOrAbsolutePath}".`
        );
      }
...
export class LocationFile {
  ...
  public constructor(private uri: string, private content: string) {}
...
export class LocationPointer {
  ...
  public constructor(private locationFile: LocationFile, offset: number) {
  ...
  public getOffset(
    vars: PointerVariations = DEFAULT_POINTER_VARIATIONS
  ): number {
  ...
  private computeLineAndCol() {
    ...
    throw new Error(
      `Reached the end of file ${this.getFile().getUri()} without reaching offset ${this.getOffset()}.`
    );
  }
...
export class LocationSpan {
  ...
  public static FromSeveral(...locationSpan: LocationSpan[]): LocationSpan {
    ...
    for (let index = 0; index < locationSpan.length; index++) {
      ...
      if (file != referenceFile) {
        throw new Error(
          `Cannot create a LocationSpan from several LocationSpans across different files (${referenceFile.getUri()}, ${file.getUri()}).`
        );
      }
  ...
  public getFile(): LocationFile {
    ...
    if (startFile != endFile) {
      throw new Error(
        `Expected start and end pointers in LocationSpan to have the same file (${startFile.getUri()}, ${endFile.getUri()}).`
      );
    }
    return startFile;
  }
