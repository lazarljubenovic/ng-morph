...
export abstract class TemplateNode extends NgAstNode {
  ...
  public getNextTemplateSibling(): TemplateNode | undefined {
    ...
    if (parent != null) {
      ...
      if (index == -1)
        throw new Error(`Expected to have found self in parent's children.`);
      ...
      if (index == -1)
        throw new Error(
          `Expected to have found self (root) in template's roots.`
        );
      const nextIndex = index + 1;
      return children[nextIndex];
    }
  ...
  public getNextTemplateSiblingOrThrow(): TemplateNode {
    return throwIfUndefined(
      this.getNextTemplateSibling(),
      `Expected to have found the next sibling.`
    );
  }

  public getDescendants<T extends TemplateNode>(
    guard: (tNode: TemplateNode) => tNode is T
  ): T[];
  ...
  public getDescendant<T extends TemplateNode>(
    guard: (tNode: TemplateNode) => tNode is T
  ): T | undefined;
  ...
  public getDescendantOrThrow<T extends TemplateNode>(
    guard: (tNode: TemplateNode) => tNode is T
  ): T;
  public getDescendantOrThrow(
    predicate?: Predicate<TemplateNode> | undefined
  ): TemplateNode;
  ...
  public getDescendantOrThrow(
    ...
    return throwIfUndefined(
      this.getDescendant(predicate),
      `Expected to find a descendant of kind.`
    );
  }

  protected _replaceTextByTokens(
    textReplaceConfigs: Iterable<TextReplaceConfig>
  ) {
    for (const { token, newText } of textReplaceConfigs) {
      ...
      this.getTemplate()._forEachTokenAfter(
        token,
        token => {
  ...
  protected getTokens<T extends Token>(
    guard: (token: Token) => token is T
  ): T[];
  protected getTokens(predicate: Predicate<Token>): Token[];
  protected getTokens(): Token[];
  protected getTokens(predicate?: Predicate<Token>): Token[] {
    return predicate == null ? this.tokens : this.tokens.filter(predicate);
  }

  protected getFirstTokenOfType<Type extends TokenType>(
    type: Type
  ): Token<Type> | undefined {
    return this.tokens.find(token => token.type == type);
  }

  protected getFirstTokenOfTypeOrThrow<Type extends TokenType>(
    type: Type
  ): Token<Type> {
    ...
    return throwIfUndefined(
      ...
      [
        ...
        `${this.tokens.map(token => token.typeName).join(', ') ||
  ...
  protected getFirstTokenIndex(): number {
    const firstToken = getFirstElementOrThrow(
      this.tokens,
  ...
  protected forEachTokenAfterHere(fn: TapFn<Token>) {
    const lastToken = getLastElementOrThrow(
      this.tokens,
      `The tokens array is empty.`
    );
    const template = this.getTemplate();
  ...
  protected forEachTokenHere(fn: TapFn<Token>) {
    ...
    const template = this.getTemplate();
    template._forEachTokenBetween(firstToken, lastToken, fn, {
      inclusiveStart: true,
      inclusiveEnd: true
    });
  }
...
export class InterpolationTemplateNode extends TemplateNode {
  ...
  public changeText(newText: string): this {
    this._replaceTextByTokens([{ token: this.getTextToken(), newText }]);
    return this;
  }
...
export abstract class ElementLikeTemplateNode extends TemplateNode {
  ...
  public constructor(
    ...
    for (const attribute of allAttributes) {
      ...
      else if (isBoundAttribute(attribute))
        this.boundAttributes.push(attribute);
      else if (isBoundEvent(attribute)) this.boundEvents.push(attribute);
      else if (isBananaInTheBox(attribute))
        this.bananaInTheBoxes.push(attribute);
      else if (isReference(attribute)) this.references.push(attribute);
      else
        throw new Error(
          `Unexpected type of attribute ${(attribute as any).constructor.name}.`
        );
    }
  }
  ...
  public getAttributes<T extends AttributeTemplateNode>(
    guard: (attr: AttributeTemplateNode) => attr is T
  ): T[];
  public getAttributes(
    predicate?: Predicate<AttributeTemplateNode> | undefined
  ): AttributeTemplateNode[];
  public getAttributes(
    predicate?: Predicate<AttributeTemplateNode>
  ): AttributeTemplateNode[] {
  ...
  public getFirstAttribute<T extends AttributeTemplateNode>(
    guard: (attr: AttributeTemplateNode) => attr is T
  ): T | undefined;
  public getFirstAttribute(
    predicate?: Predicate<AttributeTemplateNode> | undefined
  ): AttributeTemplateNode | undefined;
  public getFirstAttribute(
    predicate?: Predicate<AttributeTemplateNode>
  ): AttributeTemplateNode | undefined {
    return predicate == null
  ...
  public getFirstAttributeOrThrow<T extends AttributeTemplateNode>(
    guard: (attr: AttributeTemplateNode) => attr is T
  ): T;
  public getFirstAttributeOrThrow(
    predicate?: Predicate<AttributeTemplateNode> | undefined
  ): AttributeTemplateNode;
  public getFirstAttributeOrThrow(
    predicate?: Predicate<AttributeTemplateNode>
    ...
    return throwIfUndefined(
      this.getFirstAttribute(predicate),
      `Expected to find an attribute.`
    );
  }

  public getReferenceNamed(
    referenceName: string
  ): ReferenceTemplateNode | undefined {
    return this.references.find(
      reference => reference.getName() == referenceName
    );
  }

  public getReferenceNamedOrThrow(
    referenceName: string
  ): ReferenceTemplateNode {
    const position = this.getLocationSpan().printLong();
    const knownReferences =
      this.getAttributes(isReference)
        .map(ref => ref.getName())
        .join(', ') || '(none)';
    const error = [
