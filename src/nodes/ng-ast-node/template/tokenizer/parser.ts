...
class _TreeBuilder {
  ...
  private _consumeExpansion(token: lex.Token) {
    ...
    if (this._peek.type !== lex.TokenType.EXPANSION_FORM_END) {
      this._errors.push(
        TreeError.create(
          null,
          this._peek.locationSpan,
          `Invalid ICU message. Missing '}'.`
        )
      );
      return;
    }
    const sourceSpan = new LocationSpan(
      token.locationSpan.getStart(),
      this._peek.locationSpan.getEnd()
    );
    const node = new html.Expansion(
      [token],
      switchValue.parts[0],
      type.parts[0],
      cases,
      sourceSpan,
      switchValue.locationSpan
    );
  ...
  private _parseExpansionCase(): html.ExpansionCase | null {
    ...
    const sourceSpan = new LocationSpan(
      value.locationSpan.getStart(),
      end.locationSpan.getEnd()
    );
    const expSourceSpan = new LocationSpan(
      start.locationSpan.getStart(),
      end.locationSpan.getEnd()
    );
    const node = new html.ExpansionCase(
      [],
      value.parts[0],
      parsedExp.rootNodes,
      sourceSpan,
  ...
  private _consumeStartTag(startTagToken: lex.Token) {
    ...
    if (this._peek.type === lex.TokenType.TAG_OPEN_END_VOID) {
      ...
      if (
        ...
        this._errors.push(
          TreeError.create(fullName, startTagToken.locationSpan, errorMessage)
        );
      }
  ...
  private _consumeEndTag(endTagToken: lex.Token) {
    ...
    if (this.getTagDefinition(fullName).isVoid) {
      ...
      const error = TreeError.create(
        fullName,
        endTagToken.locationSpan,
        errMsg
      );
      this._errors.push(error);
    }
  ...
  private _consumeAttr(attrNameToken: lex.Token): html.Attribute {
    ...
    const fullName = mergeNsAndName(
      attrNameToken.parts[0],
      attrNameToken.parts[2]
    );
    ...
    const sourceSpan = new LocationSpan(
      attrNameToken.locationSpan.getStart(),
      end
    );
    const node = new html.Attribute(
      tokens,
      fullName,
      value,
      sourceSpan,
