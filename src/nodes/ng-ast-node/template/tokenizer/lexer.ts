...
export function getTokenTypeName(
  tokenType: TokenType | undefined | null
): string {
...
class _Tokenizer {
  ...
  constructor(
    ...
    const range = options.range || {
      endPos: _file.getContent().length,
      startPos: 0,
      startLine: 0,
      startCol: 0
    };
  ...
  private _endToken(parts: string[], end = this._cursor.clone()): Token {
    ...
    const sourceSpan = this._cursor.getSpan(
  ...
  private _consumeAttributeValue() {
    let value: string;
    if (
      this._cursor.peek() === chars.$SQ ||
      this._cursor.peek() === chars.$DQ
    ) {
  ...
  private _isInExpansionCase(): boolean {
    return (
      getLastElement(this._expansionCaseStack) ===
...
class PlainCharacterCursor implements CharacterCursor {
  ...
  constructor(
    fileOrCursor: LocationFile | PlainCharacterCursor,
    range?: LexerRange
  ) {
  ...
  getSpan(start?: this, leadingTriviaCodePoints?: number[]): LocationSpan {
    ...
    if (leadingTriviaCodePoints) {
      ...
      while (
        this.diff(start) > 0 &&
        leadingTriviaCodePoints.includes(start.peek())
      ) {
        start.advance();
      }
    }

    const startParseLocation = new LocationPointer(
      start.file,
      start.state.offset
    );
  ...
  protected updatePeek(state: CursorState): void {
    state.peek =
      state.offset >= this.end ? chars.$EOF : this.charAt(state.offset);
  }
}
...
class EscapedCharacterCursor extends PlainCharacterCursor {
  ...
  constructor(fileOrCursor: LocationFile, range: LexerRange);
  constructor(
    fileOrCursor: LocationFile | EscapedCharacterCursor,
    range?: LexerRange
  ) {
  ...
  protected decodeHexDigits(
    start: EscapedCharacterCursor,
    length: number
  ): number {
