...
export class TemplatePrinter {
  public print(template: Template): string {
    return template
      .getRoots()
      .map(root => this.printTemplateNode(root, 0))
      .join('\n');
  }

  private printTemplateNode(
    templateNode: TemplateNode,
    indent: number
  ): string {
    const templateChildren = templateNode.getTemplateChildren();
    const fullLine =
      prefixWithSpaces(indent, this.printLine(templateNode)) +
      ` ~ ` +
      templateNode.getLocationSpan().printMedium({ oneBased: true });
    if (templateChildren.length > 0) {
      return (
        fullLine +
        '\n' +
        templateChildren
          .map(child => this.printTemplateNode(child, indent + 1))
          .join('\n')
      );
    } else {
      return fullLine;
  ...
  private printLine(templateNode: TemplateNode): string {
    ...
    if (tntg.isInterpolation(templateNode))
      return this.printLineInterpolation(templateNode);
    if (tntg.isElementLike(templateNode))
      return this.printLineElementLike(templateNode);
  ...
  private printLineInterpolation(
    templateNode: InterpolationTemplateNode
  ): string {
  ...
  private printLineElementLike(templateNode: ElementLikeTemplateNode): string {
    ...
    const attributes = templateNode
      .getAttributes(isTextAttribute)
      .map(input => `${input.getName()} => ${input.getValue()}`)
      .join(', ');
    const inputs = templateNode
      .getAttributes(isBoundAttribute)
      .map(input => `${input.getName()} => ${input.getValue()}`)
      .join(', ');
    const outputs = templateNode
      .getAttributes(isBoundEvent)
      .map(input => `${input.getName()} => ${input.getHandler()}`)
      .join(', ');
