...
export class Component extends Declarable {
  ...
  public getTemplateLocationSpan(): LocationSpan {
    ...
    const templateString =
      inlineTemplate != null ? inlineTemplate : externalTemplate;
    return throwIfUndefined(
      templateString,
      `Expected component to have an inline or external template.`
    );
  }
  ...
  public getTemplate(): Template {
    if (this.isInlineTemplate()) {
      ...
      return Template.FromLocationSpan(
        this.project,
        locationSpan,
  ...
  public getExternalTemplatePathOrThrow(): string {
    return throwIfUndefined(
      this.getExternalTemplatePath(),
      `Expected component "${this.getName()}" to have an external template.`
    );
  }
  ...
  public getChangeDetectionStrategy(): ChangeDetectionStrategy | undefined {
    ...
    if (!TypeGuards.isIdentifier(expression))
      throw new Error(
        `Expected RHS of @Component.changeDetection to be an identifier.`
      );
    if (expression.getText() != 'ChangeDetectionStrategy')
      throw new Error(
        `Expected RHS of @Component.changeDetection to be ChangeDetectionStrategy.xxx.`
      );
    ...
    throw new Error(
      `Expected @Component.changeDetection to be "OnPush" or "Default".`
    );
  }

  public getChangeDetectionStrategyOrThrow(): ChangeDetectionStrategy {
    return throwIfUndefined(
      this.getChangeDetectionStrategy(),
      `Expected @Component.changeDetection to exist.`
    );
  }
  ...
  public getViewEncapsulation(): ViewEncapsulation | undefined {
    ...
    if (!TypeGuards.isIdentifier(expression))
      throw new Error(
        `Expected RHS of @Component.encapsulation to be an identifier.`
      );
    if (expression.getText() != 'ChangeDetectionStrategy')
      throw new Error(
        `Expected RHS of @Component.encapsulation to be ChangeDetectionStrategy.xxx.`
      );
    ...
    throw new Error(
      `Expected @Component.encapsulation to be "Emulated", "Native", "None" or "ShadowDom".`
    );
  }

  public getViewEncapsulationOrThrow(): ViewEncapsulation {
    return throwIfUndefined(
      this.getViewEncapsulation(),
      `Expected @Component.viewEncapsulation to exist.`
    );
  }
  ...
  private getDecoratorProperty<T>(
    key: string,
    guard: tg.Guard<T>,
    message: string | ((actual: string) => string)
  ): T | undefined {
    const decoratorArguments = this.getDecorator().getArguments();
    if (decoratorArguments.length != 1)
      throw new Error(
        `Expected @Component decorator to have only a single argument.`
      );
    const argument = decoratorArguments[0];
    if (!TypeGuards.isObjectLiteralExpression(argument))
      throw new Error(
        `Expected the only argument of @Component decorator to be an object literal expression.`
      );
    ...
    const errorMessage =
      message != null ? message : `getDecoratorProperty predicate failed.`;
    if (!guard(initializer))
      throw new Error(
        typeof errorMessage == 'string'
          ? errorMessage
          : errorMessage(initializer.getKindName())
      );
    return initializer;
  }

  private getDecoratorPropertyOrThrow<T>(
    key: string,
    guard: tg.Guard<T>,
    messageNotFound: string,
    messageGuardFail: string
  ) {
    return throwIfUndefined(
  ...
  private getInlineTemplateLocationSpan(): LocationSpan | undefined {
    const property = this.getDecoratorProperty(
      ...
      TypeGuards.isStringLiteral,
      kind =>
        `Expected @Component.template to be a string literal, but got ${kind}.`
    );
    if (property == null) {
      return undefined;
    } else {
  ...
  private getTemplateUrl(): string | undefined {
    const property = this.getDecoratorProperty(
      ...
      TypeGuards.isStringLiteral,
      kind =>
        `Expected @Component.templateUrl to be a string literal, but got ${kind}.`
    );
    return property == null ? undefined : property.getLiteralValue();
  }
