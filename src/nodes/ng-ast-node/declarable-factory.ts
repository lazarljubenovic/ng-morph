...
export function createDeclarable(
  project: Project,
  ngModule: NgModule,
  ...
  if (
    componentDecorators.length +
      directiveDecorators.length +
      pipeDecorators.length !=
    1
  ) {
    throw new Error(
      `Expected exactly one decorator of @Component, @Directive and @Pipe.`
    );
  }

  if (componentDecorators.length == 1)
    return new Component(project, ngModule, classDeclaration);
  if (directiveDecorators.length == 1)
    return new Directive(project, ngModule, classDeclaration);
  if (pipeDecorators.length == 1)
    return new Pipe(project, ngModule, classDeclaration);
